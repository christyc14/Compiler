...
for i in ${input_dir}/*.c ; do
  ...
  #  REF_P_OUT=$?
    ...
    if [[ ${have_compiler} -ne 0 ]] ; then
      ...
      echo -e "\e[1;31m $base, Fail, Expected ${REF_C_OUT}, got ${GOT_P_OUT}\e[0m"
    else
      echo -e "\e[1;32m $base, Pass \e[0m"
      counter_pass+=1
    fi
    counter+=1
done
echo -e "\e[1;34m Passed $counter_pass tests out of $counter\e[0m"
